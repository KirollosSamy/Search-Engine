...
import apt.project.MongoDB.InvertedFiles_DB;
...
public class Main {
    ...
    private static void printCrawlerOutFile(){
        ...
        try {
            FileWriter crawlerFile = new FileWriter("crawler.txt");
            int index=0;
            for (Map.Entry<String, Double> entry: pageRanks.entrySet())
            {
                ...
                urls_db.insert_pages(index,url,description+keywords,doc.select("title").text(),rank);
                index++;
            }

            crawlerFile.close();
        } catch (IOException e) {
            System.out.println("An error occurred in printing crawler file");
        }
    }
